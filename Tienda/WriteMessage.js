...
function composeMessage(cabecera, cuerpo) {
    ...
    switch (cabecera.tipoMensaje) {
        ...
        case 'TSC':
            composedMessage = writeBodyListaCompra(xmlDoc, cuerpo.arrayIdsProducto, cuerpo.arrayCantidad, 'TSC');
            break;
        
        case 'TLC':
            composedMessage = writeBodyListaCompra(xmlDoc, cuerpo.arrayIdsProducto, cuerpo.arrayCantidad, 'TLC');
            break;
        
        default:
            alert('Error: ' + cabecera.tipoMensaje);
            break;
    }
    ...
    xmlToSend = convertToString(composedMessage);
    alert("Esto es un XML: " + xmlToSend);
...
function writeBodyListaCompra(xmlDoc, arrayIdsProducto, arrayCantidad, tipo){
    var nodeListaCompra = xmlDoc.createElement("listaCompra");
    var cuerpo = xmlDoc.getElementsByTagName("cuerpo")[0]
    cuerpo.appendChild(nodeListaCompra);
    cuerpo.setAttribute('xsi:type', 'tipo'+tipo);
...
function writeBodyTSAT(xmlDoc, arrayTiendas, conectar){
    ...
    if (conectar) {
        ...
        arrayTiendas.map((tienda) => {
            ...
            ip_tienda.appendChild(xmlDoc.createTextNode(tienda.getIp()));
            id_tienda.appendChild(xmlDoc.createTextNode(tienda.getId()));
