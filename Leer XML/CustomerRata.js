...
class Customer {
    ...
    constructor(number, totalProduct, customerProduct, shopPerIP, hotelReference) {
		...
		this.shop = null;
		var date = new Date();
		this.startHour = date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()+":"+date.getMilliseconds();
		this.endHour = "None";
		this.messages = 0;
	...
	async main(){
			...
			if(resultado){
				...
				if(resultado){
					...
					if(resultado){
						...
						var retorno = this.processTLC(resultado, shop);
						
						////////////////////////////////////////////////////////////////////////////////////////////////
						var productos = retorno[0];
						var precios = retorno[1];
						////////////////////////////////////////////////////////////////////////////////////////////////
						...
						if(productos.length == 0){
							...
							///////////////////////////////////////////////////////////////////////////////////
							//Eliminar los undefined del array de productos
							for(var i = productos.length-1; i--;){
								if(productos[i] == null){
		...
		//Fin de la compra
		var date = new Date();
		this.endHour = date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()+":"+date.getMilliseconds();
	...
	generateShopList(numberOfShops, shopIPList, shopPerIP){
		...
		for (var i = 0; i < numberOfShops; i++){
			...
			while(!tiendaNueva){
				...
				//Id de la tienda
				var id = Math.floor(Math.random()*shopPerIP);
		...
		return new Promise(function(resolve, reject){
			...
			xhr.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					alert("Mensaje recibido: " + xhr.responseText);
	...
	askForShops(shop){
		...
		return new Promise(function(resolve, reject){
			...
			//Error de conexión
			xhr.onerror = function(){
				alert("Estoy en 353: " + xhr.responseText);
				resolve(false);
			};
	...
	processShops(xhr, shop){
		...
		//Saca las tiendas
		for(var i = 0; i++; i < procesado.ips.length){
			...
			for(var shop of this.knownShops){
				if(this.equalsShops(tienda,shop)){
					nueva = false;
					break;
				}
			}
	...
	equalsShops(shop1, shop2){
		if (!shop1.ip.equals(shop2.ip)) return false;
	...
	processTLC(xhr, shop){
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////
		//Precios de los productos, los productos tienen asociado el precio en el mismo index de este array
		for(var i = 0; i < procesado.preciosProductos.length; i++){
			precios.push(procesado.preciosProductos[i]);
		}
		////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if(productos.length > 0){
			...
			for(var producto of productos){
				this.addToLog("* Producto " + producto.getId());
			}
		}
		else{
	...
	processTSC(xhr, shop){
		...
		if(productos.length > 0){
			...
			for(var i = 0; i < productos.length; i++){
				...
				this.addToLog("* Producto " + productos[i].getId() + " (" + productos[i].getOriginalQuantity() + " unidades)");
			}
		}
		else{
