  ...
  class Tienda extends funcionesGlobales
	{
  	...
  	#Atributos
			...
			if($row != NULL){
				...
				foreach ($nombreProductos as $i => $value) {
					...
					if($row != NULL){
						...
						$precio = $row["Precio"];
						#Establecemos el precio según la cantidad disponible
						if($cant > 200){
							if($cant > 300){
								$precio = intval($precio*0.5);
							}
							else{
								$porcentajeBajada = ($cant-200)/200 #un valor de 0 a 0.5
								$precio = intval($precio*(1-$porcentajeBajada));
							}
						}
						else if($cant <= 100){
							if($cant < 20){
								$precio = intval($precio*1.5);
							}
							else{
								$porcentajeSubida = ($cant-20)/160 #un valor de 0 a 0.5
								$precio = intval($precio*(1.5-$porcentajeSubida));#En este caso restamos el porcentaje porque la proporción es inversa
							}
						}
		...
		function dameTienda($tienda, $cliente, $ipCliente, $idCliente, $idMen, $horaEntrada){
			...
			funcionesGlobales::logs("logHotel.txt", "Comprobamos tiendas conocidas");
			$mysqli = funcionesGlobales::conectBBDD();
      ...
      while($row = mysqli_fetch_assoc($compradorResult)){
            ...
            funcionesGlobales::logs("logHotel.txt", "Solicitamos el envio de mensaje");
